...
def diferential_evolution(fobj, bounds, fpenalty, mut=0.8, crossp=0.7, popsize=20, its=1000) -> (list, float):
    ...
    for i in range(its):
        for j in range(popsize):
            ...
            trial_denorm = fpenalty(trial_denorm, min_b, max_b, dimensions)
            f = fobj(trial_denorm)
            if f > fitness[j]:
                fitness[j] = f
                pop[j] = trial
...
def fpenalty(u_i, min_b, max_b, dims):
    for j in range(0, dims):
        if u_i[j] > min_b[j] and u_i[j] < max_b[j]:
            u_i[j] = min_b[j] + (max_b[j]-min_b[j])*random()
    return u_i


image = cv.imread('Image_3.bmp', cv.IMREAD_COLOR)
template = cv.imread('Template.bmp', cv.IMREAD_COLOR)
...
# print(algo([290, 260]))
# print(fpenalty([290, 260], (0, 0), (398, 577), 2))


it = list(diferential_evolution(fobj=fobj(ncc=ncc, image=image_gray,
                                          template=template_gray), bounds=bounds, fpenalty=fpenalty, popsize=70, its=1000))
...
image_result = cv.rectangle(image, (xp, yp),
                            (xp+template_w, yp+template_h), (255, 100, 100), 2)
